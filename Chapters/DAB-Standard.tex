...
The \gls{prs} is generated via a specifically-defined process outlined in the \gls{dab} standard, which uses various formulae and look-up tables~\cite{dabstandard}. The details of this process for Transmission Mode~I are included in Appendix~\ref{sect:dab-values_prs}. Note that the values of the \gls{prs} are not arbitrary: they create a so-called \gls{cazac} waveform in time, which facilitates robust detection in the presence of noise and other unwanted effects~\cite{gandy2003dab}.
